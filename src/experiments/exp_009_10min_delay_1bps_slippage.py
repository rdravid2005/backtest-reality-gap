...
EXP-009_QQQ_ORB_5M_10MIN_DELAY_1BPS_SLIPPAGE

This experiment combines two earlier findings:

1. The unrestricted 10-minute delayed-entry ORB improved performance.
2. 1 basis point of slippage significantly weakened the clean baseline.

This test asks whether the improved 10-minute delayed-entry version can survive
a modest execution-friction assumption.

Strategy:
- QQQ
- 5-minute opening range
- Long-only breakout
- Wait 10 minutes after first breakout signal
- Enter regardless of whether price is still above the opening range high
- End-of-day exit
- 1 basis point slippage per order
"""
...
class BacktestRealityGap(QCAlgorithm):
    ...
    def initialize(self):
        ...
        security = self.add_equity("QQQ", Resolution.MINUTE)
        self.symbol = security.symbol

        # 1 basis point slippage per order = 0.01%
    ...
    def on_data(self, data: Slice):
        ...
        # If a breakout signal already happened, wait until the delayed entry time.
        # Important: this version enters after 10 minutes regardless of whether
        # price is still above the opening range high.
        ...
        # First breakout signal starts the 10-minute entry delay.
